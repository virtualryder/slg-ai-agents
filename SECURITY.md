...
4. **Scoped, short-lived tokens** — per-call, request-bound, single-use (`mcp_gateway/tokens.py`). The golden-path SAM templates generate the HMAC signing key **in-stack** (`AWS::SecretsManager::Secret` with `GenerateSecretString`) — it is never committed and never passed on the CLI — and resolve it into `GATEWAY_TOKEN_SECRET`/`APPROVAL_TOKEN_SECRET` via a Secrets Manager dynamic reference, exporting its ARN as the `TokenSecretArn` stack output. **Residual limitation:** the resolved value still materializes as a Lambda environment variable (plaintext in the function configuration); a single symmetric key still signs both gateway and approval tokens. Production should have the reviewer/gateway read the key from Secrets Manager/KMS at runtime (and split the two token types onto distinct keys / an asymmetric KMS key) rather than injecting it as an env var — see `docs/INCIDENT-RESPONSE-AND-KEY-MANAGEMENT.md`.
