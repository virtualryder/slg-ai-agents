# Benefits / HHS Caseworker Assist — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 04 (04-benefits-caseworker) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `ELIGIBILITY_CASEWORKER`
- **Granted tools (gateway targets):** `eligibility.screen`, `eligibility.get_case`, `eligibility.create_application`, `eligibility.generate_notice`, `idp.extract_document`, `kb.search_policy`, `identity.verify_resident`, `consent.check`
- **Systems of record / connectors:** Integrated eligibility system · IDP · Identity · KB
- **Consequential action withheld in code:** `eligibility.adjudicate` is **not** in this agent's grants — a human role holds it (verified by an automated test).
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = ELIGIBILITY_CASEWORKER`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `INTEGRATED_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=04-benefits-caseworker`.
...
#    e.g. "Do I qualify for benefits?"
# 2. Write path: drive an action that calls eligibility.create_application; confirm it PAUSES at the gate,
...
#    Withheld: confirm NO path lets the agent call `eligibility.adjudicate`.
# 4. Accessibility: run governance/accessibility checks on a generated response.
```

## Stage H — Go-live checklist
- [ ] `ELIGIBILITY_CASEWORKER` mapping verified end-to-end via the IdP
