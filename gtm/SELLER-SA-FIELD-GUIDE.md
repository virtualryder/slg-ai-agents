...
- **Commercial regions, GovCloud-ready** is the default and deploys day-one. **GovCloud (US)** when FedRAMP High / IL-4/5 or contract terms require it — note Bedrock + Guardrails + Knowledge Bases are FedRAMP High/IL-4/5 in GovCloud, and AgentCore is GovCloud-limited, so **use the `portable` gateway path in GovCloud**. *(Precision: the **AWS services** carry these authorizations in GovCloud; the **accelerator itself is not FedRAMP/IL-authorized** — authorization is the customer's ATO process. Do not claim "the solution is FedRAMP High.")*
