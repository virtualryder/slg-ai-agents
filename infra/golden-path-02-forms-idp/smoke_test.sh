...
INPUT=$(cat ../../aws-native-reference/02-forms-idp/sample_input.json)
...
if [ -n "$TOKEN" ]; then
  ...
  aws stepfunctions send-task-success --task-token "$TOKEN" --region "$REGION" \
    --task-output '{"approval":{"approved":true,"reviewer":{"sub":"records-supervisor-1","role":"FORMS_SUPERVISOR"}}}'
fi
