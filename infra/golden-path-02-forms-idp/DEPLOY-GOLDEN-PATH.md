# Golden Path — 02 Forms & Intelligent Document Processing (one command)

Same wired pattern as the 311 reference (`infra/golden-path-311/`), applied to **Forms & Intelligent Document Processing**.
...
(`aws-native-reference/02-forms-idp/stepfunctions/02_forms_idp.asl.json`), an HTTP API with a Cognito JWT authorizer +
...
```bash
cd infra/golden-path-02-forms-idp
./deploy.sh slg-02-dev          # sam build && sam deploy
./smoke_test.sh slg-02-dev      # start execution → approve human gate → assert outcome
./destroy.sh slg-02-dev         # sam delete
```
