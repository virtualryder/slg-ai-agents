# 03-permitting-licensing — Secure Combined Deploy (P8)

Production-shaped counterpart to `infra/golden-path-03-permitting-licensing/`. **One `sam deploy`** stands up the
...
## Deploy
```bash
cd infra/golden-path-03-permitting-licensing-secure
AWS_REGION=us-east-1 ./deploy.sh slg-03-secure-dev
```
