# Permitting & Licensing — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 03 (03-permitting-licensing) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `PERMIT_TECH`
- **Granted tools (gateway targets):** `permitting.get_permit`, `permitting.check_requirements`, `permitting.create_application`, `permitting.route_review`, `gis.get_parcel`, `kb.search_policy`, `idp.extract_document`
- **Systems of record / connectors:** Permitting (Accela/Tyler) · GIS · IDP · KB
- **Consequential action withheld in code:** `permitting.issue_permit` is **not** in this agent's grants — a human role holds it (verified by an automated test).
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = PERMIT_TECH`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `PERMITTING_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=03-permitting-licensing`.
...
#    e.g. "What do I need for a building permit?"
# 2. Write path: drive an action that calls permitting.create_application; confirm it PAUSES at the gate,
...
#    Withheld: confirm NO path lets the agent call `permitting.issue_permit`.
# 4. Accessibility: run governance/accessibility checks on a generated response.
```

## Stage H — Go-live checklist
- [ ] `PERMIT_TECH` mapping verified end-to-end via the IdP
