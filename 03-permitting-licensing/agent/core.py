...
INTENTS = {'requirements': ['what do i need', 'requirement', 'required', 'do i need a permit'], 'status': ['status', 'where is my permit', 'holding'], 'apply': ['apply', 'start a permit', 'submit application', 'new permit'], 'route': ['route', 'send for review', 'route to']}
INTENT_ACTIONS = {'requirements': {'action': 'EXPLAIN_REQUIREMENTS', 'tool': 'permitting.check_requirements', 'args': {'permit_type': 'Building'}, 'write': False, 'done': 'REQUIREMENTS_PROVIDED'}, 'status': {'action': 'STATUS_LOOKUP', 'tool': 'permitting.get_permit', 'args': {'permit_id': 'PRM-2026-0481'}, 'write': False, 'done': 'STATUS_PROVIDED'}, 'apply': {'action': 'CREATE_APPLICATION', 'tool': 'permitting.create_application', 'args': {'type': 'Building'}, 'write': True, 'done': 'APPLICATION_CREATED'}, 'route': {'action': 'ROUTE_REVIEW', 'tool': 'permitting.route_review', 'args': {'permit_id': 'PRM-2026-0481', 'reviewers': ['Zoning', 'Fire']}, 'write': True, 'done': 'ROUTED'}}
PERSONAL_INTENTS = set(['status'])
DEFAULT_INTENT = 'requirements'
CONTEXT_READ_TOOL = 'permitting.check_requirements'
CONTEXT_READ_ARGS = {'permit_type': 'Building'}
