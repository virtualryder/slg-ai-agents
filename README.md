...
**The Whole-of-Government platform is not a prerequisite.** Every agent deploys **standalone** with its own complete secure architecture — own VPC + Flow Logs + Bedrock VPC endpoint (`network.yaml`), KMS CMK + Guardrail + Cognito (`security.yaml`), append-only audit + S3 Object Lock WORM (`data.yaml`), the deny-by-default gateway, and the agent — via one `scripts/deploy.sh <agent>` command, with **no WoG dependency**. Adopt the WoG orchestration layer later, **agent by agent**; the same agents plug in unchanged. See `docs/DEPLOYMENT-MODELS.md`.
