# agent/core.py — Procurement, Contracting & Grants
...
INTENTS = {'find': ['find a contract', 'vehicle', 'cooperative', 'funding', 'grant opportunity'], 'draft_rfp': ['draft', 'rfp', 'solicitation', 'write the scope', 'requirements doc'], 'compare': ['compare', 'evaluate', 'score', 'bids', 'proposals'], 'status': ['status', 'where is the solicitation', 'deadline']}
INTENT_ACTIONS = {'find': {'action': 'FIND_VEHICLES', 'tool': 'procurement.search_contracts', 'args': {}, 'write': False, 'done': 'VEHICLES_FOUND'}, 'draft_rfp': {'action': 'DRAFT_RFP', 'tool': 'procurement.draft_rfp', 'args': {}, 'write': True, 'done': 'RFP_DRAFTED'}, 'compare': {'action': 'COMPARE_BIDS', 'tool': 'procurement.compare_bids', 'args': {}, 'write': False, 'done': 'BIDS_COMPARED'}, 'status': {'action': 'STATUS_LOOKUP', 'tool': 'procurement.search_contracts', 'args': {}, 'write': False, 'done': 'STATUS_PROVIDED'}}
PERSONAL_INTENTS = set([])
DEFAULT_INTENT = 'draft_rfp'
CONTEXT_READ_TOOL = 'procurement.search_contracts'
CONTEXT_READ_ARGS = {}
WITHHELD_TOOL = 'procurement.award'   # legally consequential action the agent may NOT call
