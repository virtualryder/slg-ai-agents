...
Cross-cutting: KMS CMK · DynamoDB append-only audit · S3 Object Lock (WORM) · CloudWatch · CloudTrail · VPC. Deploy via `infra/cloudformation/quickstart.yaml` (AgentId=06-procurement-grants) + Terraform parity; commercial **and** GovCloud. Grounding & current AWS facts: repo-root `SOURCES.md`.
