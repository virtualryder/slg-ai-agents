...
4. **IdP:** the agency identity provider (SAML/OIDC) ready to federate into Cognito, with a group that will map to `custom:slg_role = PROCUREMENT_ANALYST`.
...
In Cognito, map the agency IdP group for this workflow to the custom attribute `custom:slg_role = PROCUREMENT_ANALYST`.
**Why:** the gateway authorizes `agent grant ∩ user entitlement`, so only users with `PROCUREMENT_ANALYST` can drive this agent, and only for its granted tools: `procurement.search_contracts`, `procurement.draft_rfp`, `procurement.compare_bids`, `kb.search_policy`, `idp.extract_document`.
**Verify:** sign in as a test user and decode the JWT (jwt.io or `aws cognito-idp ...`) — confirm `custom:slg_role` contains `PROCUREMENT_ANALYST`.

## 4. Point the connectors at this agent's systems (≈10 min)
Systems of record: **ERP/e-procurement · grants systems · IDP · KB**. For each, either implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, **or** set the REST base URL for the bundled adapter, e.g.:
```bash
export PROCUREMENT_BASE_URL=https://procurement.youragency.gov
```
Method signatures match the fixtures, so no agent code changes between demo and live.
**Verify:** `CONNECTOR_MODE=live python -c "from slg_agent_platform.connectors import get_connector; print(get_connector('procurement'))"` returns the HTTP adapter (not the NotImplemented stub).

## 5. Package the agent (≈5 min)
```bash
scripts/build_lambdas.sh 06-procurement-grants
aws s3 cp .build/06-procurement-grants-lambdas.zip s3://$CODE_BUCKET/
aws s3 sync infra/cloudformation s3://$CFN_BUCKET/slg --exclude "README.md"
```
**Verify:** `aws s3 ls s3://$CODE_BUCKET/06-procurement-grants-lambdas.zip` shows the object.

## 6. Deploy the agent stack (≈10 min)
```bash
scripts/deploy.sh 06-procurement-grants dev portable native
#                 ^AgentId  ^env ^GatewayMode ^DeployMode   (use 'portable' in GovCloud)
```
This nests **security** (KMS, Bedrock Guardrail, Cognito, least-privilege role), **data** (append-only DynamoDB audit, S3 Object Lock WORM, HITL table), **gateway**, and **agent** stacks for `AgentId=06-procurement-grants`.
**Verify:** `aws cloudformation describe-stacks --stack-name slg-06-procurement-grants-dev --query "Stacks[0].StackStatus"` returns `CREATE_COMPLETE`.
...
1. **Read path:** invoke the agent with a benign request — e.g. *"Draft an RFP for IT services"* — and confirm a grounded, cited response. **Verify:** response returns with citations; grounding flag true.
2. **Write path:** drive an action that calls `procurement.draft_rfp`; confirm it **PAUSES** at the gate, then approve and confirm the write commits. **Verify:** the action appears in the audit DynamoDB table with `decision=ALLOW` and an `approved_by` reviewer.
3. **Authorization:** call a tool this role is **not** entitled to. **Verify:** `decision=DENY` in the audit table.
4. **Withheld action:** confirm there is no path for the agent to call `procurement.award` (a human role holds it). **Verify:** the automated test `test_consequential_action_withheld_from_agent` passes and the tool is absent from this agent's grants.
5. **Accessibility:** run `governance/accessibility` checks on a generated response. **Verify:** no WCAG failures.

## 10. Go-live checklist
- [ ] `PROCUREMENT_ANALYST` IdP mapping verified end-to-end (step 3)
...
`aws-native-reference/06-procurement-grants/` is the Strands + Step Functions rebuild (deterministic core + 5 Lambdas + `*.asl.json` with a `waitForTaskToken` gate); see its `DEPLOY.md`.
