# Procurement, Contracting & Grants — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 06 (06-procurement-grants) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `PROCUREMENT_ANALYST`
- **Granted tools (gateway targets):** `procurement.search_contracts`, `procurement.draft_rfp`, `procurement.compare_bids`, `kb.search_policy`, `idp.extract_document`
- **Systems of record / connectors:** ERP/e-procurement · grants systems · IDP · KB
- **Consequential action withheld in code:** `procurement.award` is **not** in this agent's grants — a human role holds it (verified by an automated test).
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = PROCUREMENT_ANALYST`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `ERP_E-PROCUREMENT_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=06-procurement-grants`.
...
#    e.g. "Draft an RFP for IT services"
# 2. Write path: drive an action that calls procurement.draft_rfp; confirm it PAUSES at the gate,
...
#    Withheld: confirm NO path lets the agent call `procurement.award`.
# 4. Accessibility: run governance/accessibility checks on a generated response.
```

## Stage H — Go-live checklist
- [ ] `PROCUREMENT_ANALYST` mapping verified end-to-end via the IdP
