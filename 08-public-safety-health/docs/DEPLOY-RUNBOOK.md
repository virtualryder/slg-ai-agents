# Public Safety / Public Health Case & Report — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 08 (08-public-safety-health) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `PUBLIC_SAFETY_ANALYST`
- **Granted tools (gateway targets):** `safety.summarize_incident`, `safety.draft_report`, `phsurveillance.run_query`, `kb.search_policy`
- **Systems of record / connectors:** Incident systems · governed data lake (Lake Formation) · OpenSearch · KB
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = PUBLIC_SAFETY_ANALYST`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `INCIDENT_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=08-public-safety-health`.
...
#    e.g. "Summarize this incident"
# 2. Write path: drive an action that calls safety.draft_report; confirm it PAUSES at the gate,
...
- [ ] `PUBLIC_SAFETY_ANALYST` mapping verified end-to-end via the IdP
