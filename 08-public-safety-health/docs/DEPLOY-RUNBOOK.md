...
4. **IdP:** the agency identity provider (SAML/OIDC) ready to federate into Cognito, with a group that will map to `custom:slg_role = PUBLIC_SAFETY_ANALYST`.
...
In Cognito, map the agency IdP group for this workflow to the custom attribute `custom:slg_role = PUBLIC_SAFETY_ANALYST`.
**Why:** the gateway authorizes `agent grant ∩ user entitlement`, so only users with `PUBLIC_SAFETY_ANALYST` can drive this agent, and only for its granted tools: `safety.summarize_incident`, `safety.draft_report`, `phsurveillance.run_query`, `kb.search_policy`.
**Verify:** sign in as a test user and decode the JWT (jwt.io or `aws cognito-idp ...`) — confirm `custom:slg_role` contains `PUBLIC_SAFETY_ANALYST`.

## 4. Point the connectors at this agent's systems (≈10 min)
Systems of record: **Incident systems · governed data lake (Lake Formation) · OpenSearch · KB**. For each, either implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, **or** set the REST base URL for the bundled adapter, e.g.:
```bash
export SAFETY_BASE_URL=https://safety.youragency.gov
```
Method signatures match the fixtures, so no agent code changes between demo and live.
**Verify:** `CONNECTOR_MODE=live python -c "from slg_agent_platform.connectors import get_connector; print(get_connector('safety'))"` returns the HTTP adapter (not the NotImplemented stub).

## 5. Package the agent (≈5 min)
```bash
scripts/build_lambdas.sh 08-public-safety-health
aws s3 cp .build/08-public-safety-health-lambdas.zip s3://$CODE_BUCKET/
aws s3 sync infra/cloudformation s3://$CFN_BUCKET/slg --exclude "README.md"
```
**Verify:** `aws s3 ls s3://$CODE_BUCKET/08-public-safety-health-lambdas.zip` shows the object.

## 6. Deploy the agent stack (≈10 min)
```bash
scripts/deploy.sh 08-public-safety-health dev portable native
#                 ^AgentId  ^env ^GatewayMode ^DeployMode   (use 'portable' in GovCloud)
```
This nests **security** (KMS, Bedrock Guardrail, Cognito, least-privilege role), **data** (append-only DynamoDB audit, S3 Object Lock WORM, HITL table), **gateway**, and **agent** stacks for `AgentId=08-public-safety-health`.
**Verify:** `aws cloudformation describe-stacks --stack-name slg-08-public-safety-health-dev --query "Stacks[0].StackStatus"` returns `CREATE_COMPLETE`.
...
1. **Read path:** invoke the agent with a benign request — e.g. *"Summarize this incident"* — and confirm a grounded, cited response. **Verify:** response returns with citations; grounding flag true.
2. **Write path:** drive an action that calls `safety.draft_report`; confirm it **PAUSES** at the gate, then approve and confirm the write commits. **Verify:** the action appears in the audit DynamoDB table with `decision=ALLOW` and an `approved_by` reviewer.
...
- [ ] `PUBLIC_SAFETY_ANALYST` IdP mapping verified end-to-end (step 3)
...
`aws-native-reference/08-public-safety-health/` is the Strands + Step Functions rebuild (deterministic core + 5 Lambdas + `*.asl.json` with a `waitForTaskToken` gate); see its `DEPLOY.md`.
