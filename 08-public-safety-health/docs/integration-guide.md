...
**Systems:** Incident systems, governed data lake (Lake Formation), OpenSearch, KB. Identity is established at the edge (Cognito federating the agency IdP); verified claims (`sub`, `custom:slg_role`) are forwarded and authorized by the gateway — least-privilege intersection `agent grant ∩ user entitlement`.
