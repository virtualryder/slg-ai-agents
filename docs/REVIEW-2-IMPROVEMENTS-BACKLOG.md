...
| R2-6 | Smoke test bypassed auth + used a simple approval + didn't fail | **MOSTLY** | The production approval path is now the **authenticated reviewer service** (P7): verified-JWT reviewer, authority + SoD + single-use, server-side bound-token minting, SFN resume, audited. The local smoke still mints a bound, SoD approval via the `mint_approval.py` stand-in and exits non-zero. The full authenticated-API negative-test suite is **P9**. |
| R2-7 | Secure infra (VPC/KMS/WORM/edge) and the working app are in two separate stacks | **DONE (flagship)** | `infra/golden-path-311-secure/` is a single SAM template where **one `sam deploy`** stands up the secure baseline (isolated VPC, PrivateLink endpoints for Bedrock/DynamoDB/STS/Logs/States/Secrets/KMS, customer-managed **CMK**, S3 **Object-Lock WORM** bucket, VPC Flow Logs) AND the full app wired into it (Lambdas **in-VPC**, audit+pending tables and all log groups **CMK-encrypted**, API fronted by **CloudFront + WAF**). cfn-lint clean. The agent-independent secure block + 4 wiring changes are documented for 02–08 adoption (`DEPLOY-SECURE.md`). |
...
- **P5 — Integrate the control plane into the workflow** (R2-4, R2-5): ✅ **DONE for all 8 agents** — each `finalize.py` runs its write through the gateway (+ append-only audit), ASLs fixed (`OutputPath:$.body` + `ResultPath:$.approval`), per-agent bound-approval smoke tests. 19 workflow tests. **Remaining for full functional reality:** P6 ✅, P7 ✅, P8 ✅ (flagship); P9 remains.
...
- **P7 — Real reviewer service** (R2-6 approval): ✅ **DONE** — `slg_agent_platform.reviewer` (`ReviewerService` + `PendingApprovalStore`) + a Lambda behind the Cognito-JWT API (`GET /approvals`, `POST /approvals/{id}/decision`). It authenticates the reviewer (verified JWT), checks **authority** (reviewer must be entitled to the tool) + **separation of duties** (approver ≠ requestor), enforces **single-use** via a conditional PENDING→APPROVED/REJECTED claim, mints the **bound token SERVER-SIDE** (the reviewer never holds the secret), **resumes** the paused execution (SendTaskSuccess/Failure), and **audits** every decision (including denied attempts). `hitl_notify` parks the pending approval + task token; each template adds a `PendingApprovalsTable` (TTL) + `ReviewerFn`. Proof: `test_reviewer_service.py` (6). `mint_approval.py` remains only as an offline smoke stand-in. **Remaining:** the full authenticated-API negative-test suite is **P9**.
- **P8 — Combine secure infra + app into one deploy** (R2-7): ✅ **DONE (flagship)** — `infra/golden-path-311-secure/` provisions VPC + PrivateLink endpoints + CMK + WORM + CloudFront/WAF + the full governed agent in a single deploy (us-east-1 for the CloudFront-scoped WAF). Reusable secure block documented for 02–08.
