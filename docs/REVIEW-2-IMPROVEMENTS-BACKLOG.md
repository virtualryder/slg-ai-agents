...
- **P6 — Make one agent functionally real** (R2-3): ✅ **311 draft/check wired to Bedrock + the deployed Guardrail** (deterministic fallback; `reasoning.py`; template sets `LLM_MODE=bedrock` + `BEDROCK_GUARDRAIL_ID`; DraftFn gains `ApplyGuardrail`). The fixture connector create already runs through the gateway (P5). **Remaining:** real KB/RAG retrieval; propagate draft/check to 02–08.
