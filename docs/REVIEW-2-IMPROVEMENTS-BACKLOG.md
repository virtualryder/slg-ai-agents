...
- **P5 — Integrate the control plane into the workflow** (R2-4, R2-5): ✅ **DONE for all 8 agents** — each `finalize.py` runs its write through the gateway (+ append-only audit), ASLs fixed (`OutputPath:$.body` + `ResultPath:$.approval`), per-agent bound-approval smoke tests. 19 workflow tests. **Remaining for full functional reality:** P6 ✅, P7 ✅; P8/P9 remain.
