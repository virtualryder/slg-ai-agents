...
def test_read_intent_completes_without_write():
    s=run_until_gate({"raw_request":"upload","acting_user_claims":CLAIMS})
    assert str(s["recommended_action"]) == "RecommendedAction.EXTRACT"
    final=resume(s, APPROVAL)
    assert final["case_status"] == "EXTRACTED" and final.get("write_result") is None

def test_write_intent_maps_to_its_action_and_gates():
    s = run_until_gate({"raw_request": "assemble", "acting_user_claims": CLAIMS})
    assert str(s["recommended_action"]) == "RecommendedAction.ASSEMBLE"
    assert resume(s, approval=None)["case_status"] == "PENDING_APPROVAL"   # no write without approval
    assert resume(s, APPROVAL)["case_status"] in ("ASSEMBLED", "BLOCKED_NEEDS_APPROVER")
