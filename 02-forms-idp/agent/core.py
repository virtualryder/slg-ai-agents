...
INTENTS = {'extract': ['upload', 'document', 'attachment', 'scan', 'extract'], 'validate': ['validate', 'check', 'complete', 'missing', 'review'], 'assemble': ['assemble', 'fill', 'complete the form', 'submit form', 'apply'], 'status': ['status', 'where is my form', 'my application']}
PERSONAL_INTENTS = set(['status'])
DEFAULT_INTENT = 'assemble'
GATHER_TOOL = 'idp.extract_document'
GATHER_ARGS = {'doc_type': 'income_statement'}
WRITE_TOOL = 'idp.assemble_form'
WRITE_ARGS = {'form_id': 'FORM-1'}
WRITE_ACTION = 'ASSEMBLE'
DONE_STATUS = 'ASSEMBLED'
...
def produce(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    artifact = {"summary": "assembled form package prepared from approved source data.",
