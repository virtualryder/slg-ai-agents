# agent/core.py — Forms & Intelligent Document Processing
...
INTENTS = {'extract': ['upload', 'document', 'attachment', 'scan', 'extract'], 'validate': ['validate', 'check completeness', 'complete', 'missing field', 'incomplete'], 'assemble': ['assemble', 'fill the form', 'complete the form', 'submit form', 'apply'], 'status': ['status', 'where is my form', 'my submission']}
INTENT_ACTIONS = {'extract': {'action': 'EXTRACT', 'tool': 'idp.extract_document', 'args': {'doc_type': 'income_statement'}, 'write': False, 'done': 'EXTRACTED'}, 'validate': {'action': 'VALIDATE', 'tool': 'idp.validate_form', 'args': {'form_id': 'HUD-2025'}, 'write': False, 'done': 'VALIDATED'}, 'assemble': {'action': 'ASSEMBLE', 'tool': 'idp.assemble_form', 'args': {'form_id': 'FORM-1'}, 'write': True, 'done': 'ASSEMBLED'}, 'status': {'action': 'STATUS_LOOKUP', 'tool': None, 'args': {}, 'write': False, 'done': 'STATUS_PROVIDED'}}
PERSONAL_INTENTS = set(['status'])
DEFAULT_INTENT = 'assemble'
CONTEXT_READ_TOOL = 'idp.extract_document'
CONTEXT_READ_ARGS = {'doc_type': 'income_statement'}
