# Forms & Intelligent Document Processing — AWS Deployment Runbook (step-by-step)
...
4. **IdP:** the agency identity provider (SAML/OIDC) ready to federate into Cognito, with a group that will map to `custom:slg_role = INTAKE_SPECIALIST`.
...
In Cognito, map the agency IdP group for this workflow to the custom attribute `custom:slg_role = INTAKE_SPECIALIST`.
**Why:** the gateway authorizes `agent grant ∩ user entitlement`, so only users with `INTAKE_SPECIALIST` can drive this agent, and only for its granted tools: `idp.extract_document`, `idp.validate_form`, `idp.assemble_form`, `kb.search_policy`, `identity.verify_resident`, `crm311.create_service_request`.
**Verify:** sign in as a test user and decode the JWT (jwt.io or `aws cognito-idp ...`) — confirm `custom:slg_role` contains `INTAKE_SPECIALIST`.

## 4. Point the connectors at this agent's systems (≈10 min)
Systems of record: **IDP (Bedrock Data Automation/Textract) · KB · Identity · Consent · 311/CRM**. For each, either implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, **or** set the REST base URL for the bundled adapter, e.g.:
```bash
export IDP_BASE_URL=https://idp.youragency.gov
```
Method signatures match the fixtures, so no agent code changes between demo and live.
**Verify:** `CONNECTOR_MODE=live python -c "from slg_agent_platform.connectors import get_connector; print(get_connector('idp'))"` returns the HTTP adapter (not the NotImplemented stub).

## 5. Package the agent (≈5 min)
```bash
scripts/build_lambdas.sh 02-forms-idp
aws s3 cp .build/02-forms-idp-lambdas.zip s3://$CODE_BUCKET/
aws s3 sync infra/cloudformation s3://$CFN_BUCKET/slg --exclude "README.md"
```
**Verify:** `aws s3 ls s3://$CODE_BUCKET/02-forms-idp-lambdas.zip` shows the object.

## 6. Deploy the agent stack (≈10 min)
```bash
scripts/deploy.sh 02-forms-idp dev portable native
#                 ^AgentId  ^env ^GatewayMode ^DeployMode   (use 'portable' in GovCloud)
```
This nests **security** (KMS, Bedrock Guardrail, Cognito, least-privilege role), **data** (append-only DynamoDB audit, S3 Object Lock WORM, HITL table), **gateway**, and **agent** stacks for `AgentId=02-forms-idp`.
**Verify:** `aws cloudformation describe-stacks --stack-name slg-02-forms-idp-dev --query "Stacks[0].StackStatus"` returns `CREATE_COMPLETE`.
...
1. **Read path:** invoke the agent with a benign request — e.g. *"Assemble my benefits form"* — and confirm a grounded, cited response. **Verify:** response returns with citations; grounding flag true.
2. **Write path:** drive an action that calls `idp.assemble_form`; confirm it **PAUSES** at the gate, then approve and confirm the write commits. **Verify:** the action appears in the audit DynamoDB table with `decision=ALLOW` and an `approved_by` reviewer.
...
- [ ] `INTAKE_SPECIALIST` IdP mapping verified end-to-end (step 3)
...
`aws-native-reference/02-forms-idp/` is the Strands + Step Functions rebuild (deterministic core + 5 Lambdas + `*.asl.json` with a `waitForTaskToken` gate); see its `DEPLOY.md`.
