# Forms & Intelligent Document Processing — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 02 (02-forms-idp) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `INTAKE_SPECIALIST`
- **Granted tools (gateway targets):** `idp.extract_document`, `idp.validate_form`, `idp.assemble_form`, `kb.search_policy`, `identity.verify_resident`, `consent.*`, `crm311.create_service_request`
- **Systems of record / connectors:** IDP (Bedrock Data Automation/Textract) · KB · Identity · Consent · 311/CRM
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = INTAKE_SPECIALIST`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `IDP_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=02-forms-idp`.
...
#    e.g. "Assemble my benefits form"
# 2. Write path: drive an action that calls idp.assemble_form; confirm it PAUSES at the gate,
...
- [ ] `INTAKE_SPECIALIST` mapping verified end-to-end via the IdP
