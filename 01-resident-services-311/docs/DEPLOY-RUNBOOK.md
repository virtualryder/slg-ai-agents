# Resident Services & 311 Navigator — AWS Deployment Runbook
*Audience: AWS Solutions Architect / customer engineer. Outcome: Agent 01 (01-resident-services-311) running in a real AWS account, commercial or GovCloud. Companion to the platform runbook `runbooks/WOG-PLATFORM-DEPLOYMENT-RUNBOOK.md` (shared foundation) — this file is the agent-specific path.*
...
- **Acting role (IdP claim `custom:slg_role`):** `RESIDENT_SERVICES_AGENT`
- **Granted tools (gateway targets):** `crm311.*`, `kb.search_policy`, `identity.verify_resident`, `consent.check`, `scheduling.*`, `gis.get_parcel`
- **Systems of record / connectors:** 311/CRM · KB · Identity · Scheduling · GIS
...
In Cognito, map the agency IdP group for this workflow to `custom:slg_role = RESIDENT_SERVICES_AGENT`. The gateway authorizes `agent grant ∩ user entitlement`, so only users with this role can drive the agent — and only for this agent's granted tools.

## Stage C — Implement / point the connectors
For each system above, implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, or set `<KIND>_BASE_URL` for the bundled REST adapter (e.g. `311_CRM_BASE_URL`). Method signatures match the fixtures — no agent code changes demo→live. Validate each connector against the live system before go-live.
...
```
This nests the security (KMS, **Guardrail**, Cognito, least-privilege role), data (append-only DynamoDB audit, **S3 Object Lock WORM**, HITL table), gateway, and agent stacks for `AgentId=01-resident-services-311`.
...
#    e.g. "How do I find my trash pickup day?"
# 2. Write path: drive an action that calls crm311.create_service_request; confirm it PAUSES at the gate,
...
- [ ] `RESIDENT_SERVICES_AGENT` mapping verified end-to-end via the IdP
