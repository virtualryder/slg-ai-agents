...
4. **IdP:** the agency identity provider (SAML/OIDC) ready to federate into Cognito, with a group that will map to `custom:slg_role = IT_ANALYST`.
...
In Cognito, map the agency IdP group for this workflow to the custom attribute `custom:slg_role = IT_ANALYST`.
**Why:** the gateway authorizes `agent grant ∩ user entitlement`, so only users with `IT_ANALYST` can drive this agent, and only for its granted tools: `itsm.get_ticket`, `itsm.create_ticket`, `itsm.run_runbook (SRE-entitled approver only)`, `kb.search_policy`.
**Verify:** sign in as a test user and decode the JWT (jwt.io or `aws cognito-idp ...`) — confirm `custom:slg_role` contains `IT_ANALYST`.

## 4. Point the connectors at this agent's systems (≈10 min)
Systems of record: **ITSM (ServiceNow) · Systems Manager · Security Hub/GuardDuty · KB**. For each, either implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, **or** set the REST base URL for the bundled adapter, e.g.:
```bash
export ITSM_BASE_URL=https://itsm.youragency.gov
```
Method signatures match the fixtures, so no agent code changes between demo and live.
**Verify:** `CONNECTOR_MODE=live python -c "from slg_agent_platform.connectors import get_connector; print(get_connector('itsm'))"` returns the HTTP adapter (not the NotImplemented stub).
...
```
**Verify:** `aws s3 ls s3://$CODE_BUCKET/07-govops-service-desk-lambdas.zip` shows the object.
...
```
This nests **security** (KMS, Bedrock Guardrail, Cognito, least-privilege role), **data** (append-only DynamoDB audit, S3 Object Lock WORM, HITL table), **gateway**, and **agent** stacks for `AgentId=07-govops-service-desk`.
**Verify:** `aws cloudformation describe-stacks --stack-name slg-07-govops-service-desk-dev --query "Stacks[0].StackStatus"` returns `CREATE_COMPLETE`.
...
1. **Read path:** invoke the agent with a benign request — e.g. *"My VPN is down"* — and confirm a grounded, cited response. **Verify:** response returns with citations; grounding flag true.
2. **Write path:** drive an action that calls `itsm.create_ticket`; confirm it **PAUSES** at the gate, then approve and confirm the write commits. **Verify:** the action appears in the audit DynamoDB table with `decision=ALLOW` and an `approved_by` reviewer.
...
- [ ] `IT_ANALYST` IdP mapping verified end-to-end (step 3)
