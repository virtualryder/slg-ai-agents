# agent/core.py — Public Records / FOIA & Redaction
...
INTENTS = {'search': ['find', 'search', 'records about', 'responsive'], 'classify': ['classify', 'responsive', 'duplicate'], 'redact': ['redact', 'exemption', 'protect', 'redaction'], 'package': ['package', 'produce', 'assemble the package', 'production'], 'status': ['status', 'my request', 'clock', 'deadline']}
INTENT_ACTIONS = {'search': {'action': 'SEARCH', 'tool': 'records.search', 'args': {}, 'write': False, 'done': 'SEARCH_COMPLETE'}, 'classify': {'action': 'CLASSIFY', 'tool': 'records.classify', 'args': {'record_id': 'REC-9002'}, 'write': False, 'done': 'CLASSIFIED'}, 'redact': {'action': 'PROPOSE_REDACTION', 'tool': 'records.propose_redaction', 'args': {'record_id': 'REC-9002'}, 'write': False, 'done': 'REDACTIONS_PROPOSED'}, 'package': {'action': 'ASSEMBLE_PACKAGE', 'tool': 'records.assemble_package', 'args': {'record_count': 12}, 'write': True, 'done': 'PACKAGE_READY'}, 'status': {'action': 'STATUS_LOOKUP', 'tool': 'records.search', 'args': {}, 'write': False, 'done': 'STATUS_PROVIDED'}}
PERSONAL_INTENTS = set([])
DEFAULT_INTENT = 'search'
CONTEXT_READ_TOOL = 'records.search'
CONTEXT_READ_ARGS = {}
WITHHELD_TOOL = 'records.release'   # legally consequential action the agent may NOT call
