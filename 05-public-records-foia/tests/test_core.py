...
def test_workflow_produces_grounded_artifact():
    s=run_until_gate({"raw_request":"find","acting_user_claims":CLAIMS})
...
def test_read_intent_completes_without_write():
    s=run_until_gate({"raw_request":"find","acting_user_claims":CLAIMS})
    assert str(s["recommended_action"]) == "RecommendedAction.SEARCH"
    final=resume(s, APPROVAL)
    assert final["case_status"] == "SEARCH_COMPLETE" and final.get("write_result") is None

def test_write_intent_maps_to_its_action_and_gates():
    s = run_until_gate({"raw_request": "package", "acting_user_claims": CLAIMS})
    assert str(s["recommended_action"]) == "RecommendedAction.ASSEMBLE_PACKAGE"
    assert resume(s, approval=None)["case_status"] == "PENDING_APPROVAL"   # no write without approval
    assert resume(s, APPROVAL)["case_status"] in ("PACKAGE_READY", "BLOCKED_NEEDS_APPROVER")


def test_consequential_action_withheld_from_agent():
    from slg_agent_platform.mcp_gateway import policy
    assert "records.release" not in policy.AGENT_TOOL_GRANTS["05-public-records-foia"]
