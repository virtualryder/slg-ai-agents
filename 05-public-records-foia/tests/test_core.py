...
def test_resume_with_approval_finalizes():
    ...
    assert final["case_status"] in ("PACKAGE_READY", "PENDING_OFFICIAL", "PENDING_REVIEW", "ESCALATED")

def test_consequential_action_withheld_from_agent():
    from slg_agent_platform.mcp_gateway import policy
    assert "records.release" not in policy.AGENT_TOOL_GRANTS["05-public-records-foia"]   # human owns this

def test_finalize_never_calls_withheld_tool():
    # the agent prepares but never commits the consequential action
    s = run_until_gate({"raw_request": "find", "acting_user_claims": CLAIMS})
    final = resume(s, APPROVAL)
    assert final["case_status"] in ("PENDING_OFFICIAL", "PACKAGE_READY", "ESCALATED", "PENDING_REVIEW")
