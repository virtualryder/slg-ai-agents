...
4. **IdP:** the agency identity provider (SAML/OIDC) ready to federate into Cognito, with a group that will map to `custom:slg_role = RECORDS_TECH`.
...
In Cognito, map the agency IdP group for this workflow to the custom attribute `custom:slg_role = RECORDS_TECH`.
**Why:** the gateway authorizes `agent grant ∩ user entitlement`, so only users with `RECORDS_TECH` can drive this agent, and only for its granted tools: `records.search`, `records.classify`, `records.propose_redaction`, `records.assemble_package`, `kb.search_policy`.
**Verify:** sign in as a test user and decode the JWT (jwt.io or `aws cognito-idp ...`) — confirm `custom:slg_role` contains `RECORDS_TECH`.

## 4. Point the connectors at this agent's systems (≈10 min)
Systems of record: **Records/ECMS · OpenSearch · KB**. For each, either implement the agency adapter in `platform_core/slg_agent_platform/connectors/live.py`, **or** set the REST base URL for the bundled adapter, e.g.:
```bash
export RECORDS_BASE_URL=https://records.youragency.gov
```
Method signatures match the fixtures, so no agent code changes between demo and live.
**Verify:** `CONNECTOR_MODE=live python -c "from slg_agent_platform.connectors import get_connector; print(get_connector('records'))"` returns the HTTP adapter (not the NotImplemented stub).
...
```
**Verify:** `aws s3 ls s3://$CODE_BUCKET/05-public-records-foia-lambdas.zip` shows the object.
...
```
This nests **security** (KMS, Bedrock Guardrail, Cognito, least-privilege role), **data** (append-only DynamoDB audit, S3 Object Lock WORM, HITL table), **gateway**, and **agent** stacks for `AgentId=05-public-records-foia`.
**Verify:** `aws cloudformation describe-stacks --stack-name slg-05-public-records-foia-dev --query "Stacks[0].StackStatus"` returns `CREATE_COMPLETE`.
...
1. **Read path:** invoke the agent with a benign request — e.g. *"Find records responsive to this request"* — and confirm a grounded, cited response. **Verify:** response returns with citations; grounding flag true.
2. **Write path:** drive an action that calls `records.assemble_package`; confirm it **PAUSES** at the gate, then approve and confirm the write commits. **Verify:** the action appears in the audit DynamoDB table with `decision=ALLOW` and an `approved_by` reviewer.
3. **Authorization:** call a tool this role is **not** entitled to. **Verify:** `decision=DENY` in the audit table.
4. **Withheld action:** confirm there is no path for the agent to call `records.release` (a human role holds it). **Verify:** the automated test `test_consequential_action_withheld_from_agent` passes and the tool is absent from this agent's grants.
5. **Accessibility:** run `governance/accessibility` checks on a generated response. **Verify:** no WCAG failures.

## 10. Go-live checklist
- [ ] `RECORDS_TECH` IdP mapping verified end-to-end (step 3)
